...
double speeds[NMOTORS]={0.0,0.0,0.0,0.0};// 4电机速度值 0～10


// 控制位置电机
static const char *motorNames[NMOTORS] ={"front_right_wheel", "front_left_wheel","back_right_wheel","back_left_wheel"};
...
void updateSpeed() {   
    ...
    for (int i = 0; i < NMOTORS; ++i) {
        // 更新速度
        set_velocity_client = n->serviceClient<webots_ros::set_float>(string("/pioneer3at/") + string(motorNames[i]) + string("/set_velocity"));   
...
void cmdvelDataCallback(const geometry_msgs::Twist::ConstPtr &value)
{
    ...
    //将转换好的小车速度分量为左右轮速度
    speeds[0]  = 10.0*(2.0*linear_temp - L*angular_temp)/2.0; // front left
    speeds[1]  = 10.0*(2.0*linear_temp + L*angular_temp)/2.0; // front right
    speeds[2]  = 10.0*(2.0*linear_temp - L*angular_temp)/2.0; // back left
    speeds[3]  = 10.0*(2.0*linear_temp + L*angular_temp)/2.0; // back right
    updateSpeed();
    ROS_INFO("front left_vel:%lf, front right_vel:%lf,  back left_vel:%lf,  back right_vel:%lf",speeds[0], speeds[1],speeds[2],speeds[3]);
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < NMOTORS; ++i) {
        ...
        // 初始化2个控制位置的电机
        set_position_client = n->serviceClient<webots_ros::set_float>(string("/pioneer3at/") + string(motorNames[i]) + string("/set_position"));   
    ...
    while (ros::ok()) {   
        ...
        {  
            printf("%f",timeStepClient.call(timeStepSrv));
